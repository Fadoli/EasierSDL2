...
core::core( int sdlFlags, int imgFlags ) {
    if ( !sdlFlags ) sdlFlags = DEFAULT_SDL;
    if ( !imgFlags ) imgFlags = DEFAULT_IMG;
    SDL_Init(sdlFlags);
    IMG_Init(imgFlags);
    //InitData();
    TTF_Init();
 ...
 * @return lib_Surface* 
 */
lib_Surface * lib_SurfaceRGB ( int w, int h, int R, int G, int B )
{
    lib_Surface *tmp;
 ...
 * @return lib_Surface* 
 */
lib_Surface * lib_SurfaceRGBA ( int w, int h, int R, int G, int B, int A)
{
    lib_Surface *tmp;
    if ( (tmp = SDL_CreateRGBSurface(0, w, h, 32, 0, 0, 0, 0)) != 0 )
    {
...
lib_Surface* sdl_TextCreate (TTF_Font* Police,const char* text, int R, int G, int B)
{
    ...
    {
        return lib_SurfaceRGB(1,1,0,0,0);
    }
}
lib_Surface* sdl_UTF8Create (TTF_Font* Police,const char* text, int R, int G, int B)
{
    if ( text[0] )
    {
        SDL_Color Col;
...
lib_Surface* sdl_TextCreateShadow (TTF_Font* Police,const char* text, int R, int G, int B, int Deca)
{
    lib_Surface * Text = sdl_TextCreate(Police,text,R,G,B);
    lib_Surface * Out = sdl_TextCreate(Police,text,0,0,0);
    SDL_Rect pos;
    pos.x=-Deca;
    pos.y=-Deca;
...
lib_Surface* sdl_TextCreateMaxX (TTF_Font* Police,const char* text,int Scale, int R, int G, int B, int MX)
{
    ...
    lib_Surface *Out = lib_SurfaceRGBA(MX,Ligne*Scale,0,0,0,0);
